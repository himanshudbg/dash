 ...
 *
 * The command variant also accepts a shell variable in the port position:
 * current Dash writes its hooks as guarded curl commands that read the port
 * from a file into `$P` at runtime (older versions read `$DASH_HOOK_PORT`
 * from the env) rather than baking a number, so the brand-loss fallback must
 * recognize those shapes too. (The url-field variant stays digits-only — http
 * hooks always carry a concrete port.)
 */
const DASH_URL_FULL_RE = /^https?:\/\/127\.0\.0\.1:\d+\/hook\/([a-z-]+)(\?|$)/i;
const DASH_URL_SUBSTR_RE = /https?:\/\/127\.0\.0\.1:(?:\d+|\$DASH_HOOK_PORT|\$P)\/hook\/([a-z-]+)/i;
